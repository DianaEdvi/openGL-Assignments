...
A2solution::A2solution() {
    std::cout << "A2Solution created" << std::endl;
}


// Taken from Capsule 2
int A2solution::compileAndLinkShaders(const char* vertexShaderSource, const char* fragmentShaderSource)
{
...
void A2solution::createRenderingData(const Model& model, unsigned int& VAO, unsigned int& VBO, int& vertexCount)
{
...
void A2solution::assignUniforms(int shaderProgram, const Model& model, unsigned int& VAO){
...
glm::vec3 A2solution::calculateBarycentricWeights(const glm::vec3& P, const glm::vec3& A, const glm::vec3& B, const glm::vec3& C){
    ...
    float areaPCA = (C.x - P.x) * (A.y - P.y) - (A.x - P.x) * (C.y - P.y);    
    
    if (abs(areaABC) < 0.000001f) return glm::vec3(-1, -1, -1); // avoid division by 0
