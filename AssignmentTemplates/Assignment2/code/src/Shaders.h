...
const char* getVertexShaderSource()
{
    ...
    "layout (location = 4) in vec3 aCenterWorld;"
    ...
    "out vec3 inCenterWorld;"
    "out float inRadius;"
    ""
    "void main()"
    "{"
        "barycentric = aBarycentric;"
        "inCenterWorld = (modelview * vec4(aCenterWorld, 1)).xyz;"
        "inRadius = aRadius;"
...
const char* getCircleFragmentShaderSource()
{
    ...
    "{"
        ...
        "float lambertian = max(dot(N,L), 0.0);" 
        "float distanceToBarycentric = distance(inCenterWorld, fragPos);"

        // if the barycentric coordinate is outside the circle, color blue
        "if (distanceToBarycentric > inRadius){"
